// Tier 2 with Parameterized EHT Prior for Sensitivity Analysis
//
// This model is identical to tier2_environment.stan EXCEPT:
//   - EHT_sd_prior is passed as DATA, not estimated
//   - This allows systematic sensitivity analysis: "What sigma_EHT is required to achieve +/-30 years?"
//
// Purpose: Pre-empt the counterargument "your priors are too diffuse"
...
//
// UPDATED 2025-12-03: Created for prior sensitivity analysis
// UPDATED 2026-02-16: Standardized truncation to use unit_to_truncnorm(),
...
data {
  ...
  // SENSITIVITY PARAMETER: EHT prior SD passed as data
  // This is the KEY difference from tier2_environment.stan
  real<lower=0> EHT_sd_prior;  // Fixed sigma_EHT for this sensitivity run
}
...
parameters {
  ...
  // NOTE: EHT_sd is NOT a parameter - it's fixed at EHT_sd_prior (passed as data)
...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    // KEY: Uses EHT_sd_prior (fixed data) instead of estimated EHT_sd
    EHT[i] = unit_to_truncnorm(EHT_unit[i], EHT_mean[study_area[i]], EHT_sd_prior, EHT_lower, EHT_upper);
...
model {
  ...
  // NOTE: EHT_sd is fixed at EHT_sd_prior - no prior needed
...
generated quantities {
  ...
  // Store the fixed EHT_sd for output
