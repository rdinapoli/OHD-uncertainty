// Tier 2 with Parameterized Ea Prior for Sensitivity Analysis
//
// This model fixes Ea_sigma as DATA to test activation energy sensitivity.
...
data {
  ...
  // SENSITIVITY PARAMETER: Ea prior SD passed as data
...
parameters {
  ...
  real<lower=0> H2Ot_sigma;

  real<lower=79000,upper=86000> Ea_mu;
  // Ea_sigma is NOT a parameter - fixed at Ea_sigma_prior
...
transformed parameters {
  ...
  for (i in 1:N) {
    H2Ot[i] = H2Ot_mu + H2Ot_sigma * H2Ot_raw[i];
    // KEY: Ea uses fixed sigma from data
...
model {
  ...
  // No prior on Ea_sigma - it's fixed
